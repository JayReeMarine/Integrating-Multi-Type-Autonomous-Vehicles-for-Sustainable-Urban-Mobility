...
\begin{proof}
The cost matrix $\mathbf{W}$ encodes, for every virtual slot--PV pair, the negated towed distance ($-d_{ij}^{curr}$) if the pair is feasible and $+\infty$ otherwise. The LSAP is to find a matching of slots to PVs minimizing total cost. Because the Jonker--Volgenant algorithm solves LSAP exactly~\cite{scipy2020,burkard2012assignment}, the returned matching maximizes total towed distance for the current cost matrix. Convergence follows from the fact that each accepted assignment strictly advances at least one PV's left boundary $\ell_j$, so the number of iterations is bounded by the total number of feasible segments.
\end{proof}
