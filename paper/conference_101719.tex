...
\label{lem:critical}
The point-wise capacity constraint \eqref{eq:capacity} can be verified by checking only the finite set of critical points $\mathcal{C}_i = \{e_i^a, x_i^a\} \cup \{cp_{ij}, dp_{ij} : j \in [M]\}$ for each AV $a_i$.
\end{lemma}

\begin{proof}
The number of PVs towed by AV $a_i$ is a step function over $x \in [e_i^a, x_i^a]$, changing only at coupling points (increment) and decoupling points (decrement). Between consecutive critical points, the count is constant. Thus, if capacity is satisfied at all critical points, it is satisfied everywhere.
\end{proof}
