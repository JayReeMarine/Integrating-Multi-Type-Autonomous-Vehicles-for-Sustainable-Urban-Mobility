...
cells_m1 = {}
for r in M:
    cells_m1.setdefault((int(r["seed"]), float(r["ratio"])), {})[(r["algorithm"], r["time_on"] == "True")] = float(r["saving_percent"])
cells_m1 = {k: v for k, v in cells_m1.items() if len(v) == 4}
n_seeds = len({s for s, _ in cells_m1})
mr = sorted({r for _, r in cells_m1})
def agg(key_fn):
    out = {}
    for ratio in mr:
        vals = [key_fn(v) for (s, r), v in cells_m1.items() if r == ratio]
        out[ratio] = (st.mean(vals), st.stdev(vals) if len(vals) > 1 else 0.0)
    return out
gap_on = agg(lambda v: v[("ILA", True)] - v[("greedy", True)])
gap_off = agg(lambda v: v[("ILA", False)] - v[("greedy", False)])
sav = {k: agg(lambda v, k=k: v[k]) for k in [("greedy", True), ("ILA", True), ("ILA", False)]}
...
a.errorbar(mr, [gap_on[r][0] for r in mr], yerr=[gap_on[r][1] for r in mr], fmt="o-", color="#d62728", ms=7, lw=1.8,
           capsize=3, label=f"M1 SUMO, time ON ({n_seeds} seeds, mean ± sd; placeholder demand)", zorder=4)
a.plot(mr, [gap_off[r][0] for r in mr], "s--", color="#d62728", ms=5, lw=1, alpha=.5,
...
b.plot(mr, [sav[("greedy", True)][r][0] for r in mr], "o-", color="#f4a582", mec="#d62728", label="M1 greedy, time ON")
b.plot(mr, [sav[("ILA", True)][r][0] for r in mr], "^-", color="#d62728", ms=6, label="M1 ILA, time ON")
b.plot(mr, [sav[("ILA", False)][r][0] for r in mr], "--", color="#d62728", alpha=.4, label="M1 both, time OFF")
...
fig.suptitle("Same algorithms, same τ = 5 model units — only the instance distribution differs. "
             f"M1 = {n_seeds} seeds (random labels) on randomTrips placeholder demand.", fontsize=9)
fig.tight_layout()
